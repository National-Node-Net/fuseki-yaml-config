...
public class ConfigStruct {
    ...
    public static final String TDB_2_UNION_DEFAULT_GRAPH = "tdb2:unionDefaultGraph";
    ...
    public boolean checkConnectorMismatch() {
        if(this.connectors != null) {
            ...
            if (checkServiceConnector(servicesFromConnectors)) {
                return false;
            }
    ...
    private boolean checkServiceConnector(Set<String> servicesFromConnectors) {
    ...
    public void checkDatabaseTypes() {
        for (Database database : this.databases) {
            ...
            if(database.settings() != null)
                if(database.settings().get(TDB_2_UNION_DEFAULT_GRAPH) != null)
                    if (!(String.valueOf(database.settings().get(TDB_2_UNION_DEFAULT_GRAPH)).equals("true")) && !(String.valueOf(database.settings().get(TDB_2_UNION_DEFAULT_GRAPH)).equals("false")))
                        throw new IllegalArgumentException("The value of '" +TDB_2_UNION_DEFAULT_GRAPH+ "' in " + database.name() + " settings is not a boolean");
        }
    }
